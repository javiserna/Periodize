...
if __name__ == "__main__":
	...
	minP=float((sys.argv[2]))
	maxP=float((sys.argv[3]))
	np1=int((sys.argv[4]))
	nmc1=int((sys.argv[5]))
	time=lc['time']
#	flux=lc['mag']
...
#	periods = np.geomspace(minP, maxP, num=np1) #100 steps in the period range. May you consider to change by 1000 in particular cases (increase the computing time).
#	periods = np.sort(rng.random((np1,)))*(maxP-minP)+minP
#	periods = np.linspace(minP, maxP, num=np1) #100 steps in the period range. May you consider to change by 1000 in particular cases (increase the computing time).

#  added by JH Aug 11, 2023	
	n=nmc1 # Synthetic light curve number for Bootstrap
...
# 
	...
	Prot=np.array(Prot)
	if np.std(Prot)==0:
		print(u'Period=%.6f days\n' %(np.median(Prot)))
		print(u'Period=%.6f days\n' %(np.mean(Prot)))
		print("Uncertainty quite low, please consider 1000 steps for the period range!")
	if np.std(Prot)!=0:
# JH if you want median and MAD.
		print(u'Period(med)=%.6f\u00B1%.6f days' %(np.median(Prot),1.4826*scipy.stats.median_abs_deviation(Prot)))
		print(u'Period(mean)=%.6f\u00B1%.6f days' %(np.mean(Prot),np.std(Prot)))
